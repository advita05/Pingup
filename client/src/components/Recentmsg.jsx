...
const Recentmsg = () => {
  ...
  return (
    <div className="bg-white max-w-xs mt-4 p-4 min-h-20 rounded-md shadow text-xs text-slate-800">
      <h3 className="font-semibold text-slate-800 mb-4">Recent msgs</h3>
      <div className="flex flex-col max-h-56 overflow-y-scroll no-scrollbar">
        {messages.map((message, index) => (
          <Link to={`/messages/${message.from_user_id._id}`}
            key={index}
            className="flex items-center gap-2 py-2 hover:bg-slate-100 px-2 rounded-md"
          >
            <img
              src={message.from_user_id.profile_picture}
              alt={message.from_user_id.username}
